...
{ alertId, level:'red|yellow|green', student, kp, type,
  trigger, trendData:[...], trendXAxis:[...], trendDays:14, status:'open|reviewed|ignored', note }
```
> 由 `services/alert_detector.py` 按真实答题数据生成：掌握率 <50 → red，50~60 → yellow，≥60 删除告警。
> 判定门槛：该知识点至少作答过 2 道**不同**题（与靶向练习/薄弱点同一门槛，常量 `ENGAGE_MIN`）；正确率为加权口径（主 KP 1.0 / 其它标签 0.3，见 `services/scoring.py`）。
> `trendData`/`trendXAxis`（2026-09-24 起）：近 14 天**按天**的「截至当天」累计正确率 —— 当天没做题则沿用前一天的值，第一次作答之前为 `null`（图上留空）。教师端「预警详情」用它画曲线，替代原先"最近 5 次作答的对错"。
