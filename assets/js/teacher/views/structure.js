...
const TeacherStructure = {
  ...
  _paint() {
    ...
    // 章节详情入口（点击章节头部或其「⋯」按钮；操作集成在详情页）
    U.$$('#stTree [data-ch-open]').forEach(b => b.addEventListener('click', e => {
      e.stopPropagation();
      this.openChapterDetail(b.dataset.chOpen);
    }));
    // 知识点详情入口（点击知识点标签 → 可编辑 / 删除；默认章节=标签所在的当前章节）
    U.$$('#stTree [data-kp-open]').forEach(b => b.addEventListener('click', e => {
      e.stopPropagation();
      const kp = this._findKp(b.dataset.kpOpen);
      if (kp) this.openKpDetail(kp, b.dataset.kpCh || '');
    }));
  ...
  _chapterHtml(ch) {
    ...
    }).join('') || `<p class="fz-11 t-dim" style="padding:6px 0 8px 20px">目录下暂无资源 · 点章节「⋯」加入</p>`;

    const kpChips = (ch.kps || []).length
      ? (ch.kps || []).map(k => `
        <span class="chip" style="display:inline-flex;align-items:center;gap:4px;cursor:pointer"
              data-kp-open="${U.esc(k.id)}" data-kp-ch="${U.esc(ch.id || '')}" title="点击查看 / 编辑知识点">
    ...
    return `
      <div class="list__section" data-ch="${U.esc(ch.id || '')}"
        ...
        <div class="list__item" data-ch-open="${U.esc(ch.id || '')}" style="padding-left:0;cursor:pointer">
          <div class="list__main">
          ...
          <div class="row" style="gap:4px;flex-shrink:0">
            <button class="btn btn--xs btn--ghost" data-ch-open="${U.esc(ch.id || '')}" title="章节详情：改名 / 上传 / 添加知识点 / 删除">⋯</button>
          </div>
        </div>
        ${res}
  ...
  /** 按 kpId 在缓存中定位知识点对象（章节 kps 或全局 kpOptions） */
  _findKp(kpId) {
    for (const c of (this.data.chapters || [])) {
      const k = (c.kps || []).find(x => x.id === kpId);
      if (k) return k;
    }
    return ((this.data && this.data.kpOptions) || []).find(k => k.id === kpId) || null;
  },

  /** 章节详情页：上传 / 添加知识点 / 改名 / 删除 全部集成在此 */
    ...
    Modal.open({
      ...
      body: `<div class="stack" style="gap:12px">
        <label class="stack" style="gap:4px"><span class="fz-12 t-dim">章节名称</span>
          <input class="input" id="cdName" value="${U.esc(ch.name)}"></label>
        ...
        <p class="fz-12 t-dim">资源 ${ch.resources ? ch.resources.length : 0} 个 · 知识点 ${(ch.kps || []).length} 个</p>
      </div>`,
      footer: `<button class="btn btn--outline" id="cdUpload" type="button">${icon('upload')} 上传资源</button>
        <button class="btn btn--outline" id="cdKp" type="button">${icon('plus')} 添加知识点</button>
        <button class="btn btn--danger" id="cdDel" type="button">删除章节</button>
        <button class="btn" data-close>取消</button>
        <button class="btn btn--primary" id="cdSave" type="button">保存改名</button>`,
      onMount(ov, close) {
        U.$('#cdSave', ov).addEventListener('click', () => {
          ...
          API.teacher.updateChapter(ch.id, { name, hours }).then(() => {
            close(); TeacherStructure.render(); Toast.ok('章节已更新');
          }).catch(err => Toast.error('失败', (err && err.message) || ''));
        });
        U.$('#cdUpload', ov).addEventListener('click', () => { close(); TeacherStructure.openUploadModal(ch.id); });
        U.$('#cdKp', ov).addEventListener('click', () => { close(); TeacherStructure._kpModal(ch.id); });
        U.$('#cdDel', ov).addEventListener('click', () => { close(); TeacherStructure._deleteChapter(ch.id); });
      }
    });
  },

  /**
   * 知识点详情页：可编辑（名称 / 换章 / 学时 / 重点 / 知识点介绍）+ 删除
   * @param {object} kp 知识点
   * @param {string} [chapterId] 该标签所在的「当前章节」——打开时默认选中它；
   *   未传（如从其它入口调用）时回退按知识点自身 chapter 名匹配；两者都没有则显示「未分章」
   *   （不再默默选中第一章，避免误改所属章节）
   */
  openKpDetail(kp, chapterId) {
    const chapters = ((this.data && this.data.chapterOptions) || []).filter(c => c.id);
    const curId = (chapterId && chapters.some(c => c.id === chapterId))
      ? chapterId
      : (((chapters.find(c => c.name === kp.chapter) || {}).id) || '');
    const chOpts = chapters.map(c =>
      `<option value="${U.esc(c.id)}" ${c.id === curId ? 'selected' : ''}>${U.esc(c.name)}</option>`).join('');
    Modal.open({
      title: '知识点详情',
      body: `<div class="stack" style="gap:12px">
        <label class="stack" style="gap:4px"><span class="fz-12 t-dim">知识点名称</span>
          <input class="input" id="kdName" value="${U.esc(kp.name)}"></label>
        <label class="stack" style="gap:4px"><span class="fz-12 t-dim">所属章节</span>
          <select class="select" id="kdChapter">
            ${curId ? '' : '<option value="" selected>（未分章）</option>'}
            ${chOpts}
          </select></label>
        <label class="stack" style="gap:4px"><span class="fz-12 t-dim">学时</span>
        ...
        <label class="stack" style="gap:4px"><span class="fz-12 t-dim">知识点介绍</span>
          <textarea class="input" id="kdIntro" rows="5" placeholder="简要说明该知识点的定义、用途与学习要点（会写入数据库，并在学生端/教师端图谱详情中展示）">${U.esc(kp.summary || '')}</textarea></label>
        <label class="row" style="gap:6px"><input type="checkbox" id="kdKey" ${kp.isKey ? 'checked' : ''}> <span>标记为重点</span></label>
      </div>`,
      footer: `<button class="btn btn--danger" id="kdDel" type="button">删除</button>
        <button class="btn" data-close>取消</button>
        <button class="btn btn--primary" id="kdSave" type="button">保存</button>`,
      onMount(ov, close) {
        U.$('#kdSave', ov).addEventListener('click', () => {
          const name = U.$('#kdName', ov).value.trim();
          if (!name) { Toast.warn('请输入知识点名称'); return; }
          const chId = U.$('#kdChapter', ov).value;
          const ch = chapters.find(c => c.id === chId);
          API.teacher.updateKp(kp.id, {
            name,
            chapterId: chId,
            chapter: ch ? ch.name : kp.chapter,
            hours: parseInt(U.$('#kdHours', ov).value, 10) || 0,
            isKey: U.$('#kdKey', ov).checked,
            summary: (U.$('#kdIntro', ov).value || '').trim(),
          }).then(() => { close(); TeacherStructure.render(); Toast.ok('知识点已更新'); })
            .catch(err => Toast.error('保存失败', (err && err.message) || ''));
        });
        U.$('#kdDel', ov).addEventListener('click', () => { close(); TeacherStructure._deleteKp(kp.id, kp.name); });
      }
    });
  },
    ...
    Modal.open({
      ...
      body: `
        <div class="stack" style="gap:12px">
          ...
          <label class="stack" style="gap:4px"><span class="fz-12 t-dim">知识点介绍</span>
            <textarea class="input" id="kpNewIntro" rows="4" placeholder="简要说明该知识点的定义、用途与学习要点（会写入数据库，并在学生端/教师端图谱详情中展示）"></textarea></label>
      ...
      onMount(ov, close) {
        U.$('#kpNewSave', ov).addEventListener('click', () => {
          ...
          API.teacher.createKp({
            ...
            summary: (U.$('#kpNewIntro', ov).value || '').trim(),
          }).then(() => {
            close();
            TeacherStructure.render();
